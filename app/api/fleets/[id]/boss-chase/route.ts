...
// Envoie TOUTE la flotte prendre le boss galactique en chasse — code
// CAPITAINE, même principe que POST /api/fleets/[id]/chase. Un vaisseau
// déjà occupé (rencontre en cours, action de surface, endommagé) est
// simplement ignoré plutôt que de faire échouer tout l'ordre.
export async function POST(request: Request, ctx: RouteContext<"/api/fleets/[id]/boss-chase">) {
  ...
  const fleetRows = await db.sql<{ id: string; name: string; faction: string; captain_code: string | null }>`
